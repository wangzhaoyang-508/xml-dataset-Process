...
def voc2yolo(path, classes):
    ...
    train_list = [item.split('/')[-1] for item in train_name_list]
    print('train_name_list', train_list)
    ...
    for jpg in tqdm(os.listdir(default_voc_path+'/JPEGImages/')):
        if os.path.splitext(jpg)[0] in train_list:
            print('os.path.splitext(jpg)[0]================', os.path.splitext(jpg)[0])
    ...
    for xml in tqdm(os.listdir(default_voc_path+'/Annotations/')):
        if os.path.splitext(xml)[0] in train_list:
    ...
    for filename in tqdm(os.listdir(path)):
        ...
        if filename.lower().endswith(('.png', '.jpeg')):
            ...
            with Image.open(image_path) as image:
                new_file_path = os.path.join(path, file_root + '.jpg')
                image.convert('RGB').save(new_file_path, 'JPEG')

            # 如果原始文件不是.jpg格式，则删除原始文件
